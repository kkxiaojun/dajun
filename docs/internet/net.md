...
<img :src="$withBase('/image/http/daotu.png')" alt="foo">
...
<img :src="$withBase('/image/http/002.png')" alt="foo">

<img :src="$withBase('/image/http/003.png')" alt="foo">
...
<img :src="$withBase('/image/http/004.png')" alt="foo">
...
- 3XX 开头的 HTTP 状态码都表示重定向的响应。
- 301、308 是永久重定向；302、303、307 是临时重定向。
- 301、302 是 http 1.0 的内容，303、307、308 是 http1.1 的内容。
- 301 和 302 本来在规范中是不允许重定向时改变请求 method 的（将 POST 改为 GET），实际许多浏览器实现的时候允许重定向时改变请求 method。
- 303 的出现是允许重定向时改变请求 method。此外 303 响应禁止被缓存。307、308 则不允许重定向时改变请求 method。
...
<img :src="$withBase('/image/http/005.png')" alt="foo">
...
<img :src="$withBase('/image/http/006.png')" alt="foo">
...
<img :src="$withBase('/image/http/007.png')" alt="foo">

用 telnet 抓到的响应如下:
<img :src="$withBase('/image/http/008.png')" alt="foo">
...
<img :src="$withBase('/image/http/009.png')" alt="foo">

<img :src="$withBase('/image/http/010.png')" alt="foo">
...
<img :src="$withBase('/image/http/011.png')" alt="foo">
...
<img :src="$withBase('/image/http/012.png')" alt="foo">

刚开始你可能会比较懵，先别着急，过一遍下面的流程再来看会豁然开朗。

1. client: 浏览器发送 client_random、TLS 版本、加密套件列表(TLS_ECDHE_WITH_AES_128_GCM_SHA256)。数字签名（私钥加密，公钥解密，对比哈希摘要）
2. server: 服务器发送 server_random、server_params、确认 TLS 版本、使用的加密套件列表（TLS_ECDHE_WITH_AES_128_GCM_SHA256）、服务器使用的证书
3. client:

   1. 验证服务端传来的<font color="red">证书</font>和<font color="red">签名</font>是否通过, 通过则传递参数`client_params`给服务器。
   2. 接着客户端通过<font color="red">`ECDHE`</font>算法计算出<font color="red">`pre_random`</font>, 需传入两个参数：`client_params`和`server_params`。
   3. 最后，根据`client_random`、`server_random`和`pre_random`通过一个伪随机函数计算出`secret`

4. server: 使用`ECDHE`算法生成`pre_random`，接着用和客户端相同的伪随机函数计算出`secret`

#### step 1: Client Hello

首先，浏览器发送 client_random、TLS 版本、加密套件列表。

client_random 是什么？用来最终生成 secret 的一个参数。
...
意思是`TLS`握手过程中，使用`ECDHE`算法生成`pre_random`(这个数后面会介绍)，128 位的`AES`算法进行对称加密，在对称加密的过程中使用主流的`GCM`分组模式，因为对称加密中很重要的一个问题就是如何分组。最后一个是哈希摘要算法，采用`SHA256`算法（还有 Sha384）。
...
<img :src="$withBase('/image/http/013.png')" alt="foo">
...
<img :src="$withBase('/image/http/014.png')" alt="foo">
...
<img :src="$withBase('/image/http/015.png')" alt="foo">
...
<img :src="$withBase('/image/http/016.png')" alt="foo">
...
<img :src="$withBase('/image/http/quic.png')" alt="foo">

1. HTTP/2 的连接需要 3 RTT
2. QUIC 从请求连接到正式接发 HTTP 数据一共花了 1 RTT，这 1 个 RTT 主要是为了获取 `Server Config`，后面的连接如果客户端缓存了 Server Config，那么就可以直接发送 HTTP 数据，实现 0 RTT 建立连接

###

参考:

[《web 协议详解与抓包实战——陶辉》](https://time.geekbang.org/course/intro/175)
...
[前端开发者必备的 Nginx 知识 ——conardli](https://juejin.im/post/5c85a64d6fb9a04a0e2e038c)
