...
### 真实握手
**前置知识点**
> 在TCP连接中，标志用于指示特定的连接状态或提供一些其他有用的信息（例如，故障排除目的）或处理对特定连接的控制。最常用的标志是“ SYN”，“ ACK”和“ FIN”。每个标志对应于1bt信息。

* SYN(synchronous建立联机)报文, 同步序列编号
* ACK(acknowledgement 确认)报文，用来应答的（确认号）；确认值(Acknowledgement)，为1便是确认连接
...
1. client主动发送，SYN = 1（用来同步的）报文，初始化ISN（seq=x），status: SYN_SEND(同步-已发送状态)

2. server接收到SYN, 发送确认报文（ ACK=1，SYN=1，确认号：ack=x+1）到client, 初始化 seq=y， status: SYN_REVD（同步-已接收状态）

3. client 收到 SYN 报文之后，会发送一个 ACK=1，ack=y+1，seq=x+1报文，status: ESTABLISHED。服务器收到 ACK 报文之后，也处于 ESTABLISHED

另外需要提醒你注意的是，从图中可以看出，SYN 是需要消耗一个序列号的，下次发送对应的 ACK 序列号要加 1，

为什么呢？只需要记住一个规则:

> 凡是需要对端确认的，一定消耗 TCP 报文的序列号。

下次发送对应的ACK，SYN不消耗一个序列号，无法判断是否是之前的ACK
...
1. client发送FIN报文，报文中指定seq = u, 停止再发送数据。 status: FIN_WAIT1
2. server收到FIN报文后，发送ACK报文（ACK报文确认号: u+1), TCP处于半关闭状态, client收到确认后，status: FIN_WAIT2。 status: CLOSE_WAIT
3. 如果server想断开连接了，发送FIN报文，指定序列号。 status: LAST_ACK
4. client接收到FIN后，发送ACK报文应答(需要过一阵子以确保服务端收到自己的 ACK 报文之后才会进入 CLOSED 状态)，client status: TIME_WAIT。 server: 接收到ACK，status: CLOSED
...
简单理解：要发送ACK和FIN两个报文（MSL，报文最大生存时间），两个报文，所以需要2MSL
...
过程简介：
1. 慢启动：发送方向接收方发送 1 个单位的数据，收到对方确认后会发送 2 个单位的数据，然后依次是 4 个、8 个……呈指数级增长，这个过程就是在不断试探网络的拥塞程度，超出阈值则会导致网络拥塞；
2. 拥塞避免：指数增长不可能是无限的，到达某个限制（慢启动阈值）之后，指数增长变为线性增长；
3. 快速重传：发送方每一次发送时都会设置一个超时计时器，超时后即认为丢失，需要重发；
4. 快速恢复：在上面快速重传的基础上，发送方重新发送数据时，也会启动一个超时定时器，如果收到确认消息则进入拥塞避免阶段，如果仍然超时，则回到慢启动阶段。
